...
import nifti from 'nifti-reader-js'; // Import nifti-reader-js
...
export default function Home() {
  ...
  const handleFileChange = async (event) => {
    ...
    try {
      // Read the NIfTI file using FileReader
      const reader = new FileReader();
      reader.onloadend = () => {
        const arrayBuffer = reader.result;
        try {
          // Parse the NIfTI file
          const niftiData = nifti.parseHeader(new Uint8Array(arrayBuffer)); // Use Uint8Array instead of ArrayBuffer directly
          const niftiImage = nifti.getImage(new Uint8Array(arrayBuffer), niftiData);
          const imageData = niftiImage.data; // This is the image data

          // For simplicity, display the first slice as preview (2D)
          const sliceData = imageData.slice(0, niftiData.dims[1] * niftiData.dims[2]); // Extract first slice
          const previewUrl = URL.createObjectURL(new Blob([new Uint8Array(sliceData)]));

          setPreview(previewUrl); // Set this as the preview
        } catch (error) {
      ...
      alert('Failed to process the NIfTI file.');
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div>
      ...
      <div className="file-upload">
        ...
        {/* Progress Bar */}
        {uploadProgress > 0 && <div className="progress-bar" style={{ width: `${uploadProgress}%`, height: '10px', backgroundColor: '#4caf50', marginBottom: '10px' }}></div>}
